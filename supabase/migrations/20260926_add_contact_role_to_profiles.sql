/*
# Migration: Add 'contact' role to profiles table constraint and sync RPCs

## Purpose
1. Allows 'contact' value in profiles.role CHECK constraint for partner contact persons.
2. Updates update_user_platform_role RPC to accept 'contact' role.
3. Updates accept_partner_invitation RPC to set profiles.role = 'contact' for non-owner partner members.
4. Migrates existing partner_users members so their profiles.role is updated from 'user' to 'contact'.
*/

-- 1. Drop and re-add profiles_role_check constraint with 'contact'
ALTER TABLE profiles DROP CONSTRAINT IF EXISTS profiles_role_check;
ALTER TABLE profiles ADD CONSTRAINT profiles_role_check CHECK (role = ANY (ARRAY['user'::text, 'editor'::text, 'partner'::text, 'school'::text, 'teacher'::text, 'contact'::text, 'admin'::text]));
...
BEGIN
  ...
  IF new_role NOT IN ('user', 'editor', 'partner', 'school', 'teacher', 'contact', 'moderator', 'admin') THEN
...
-- 3. Update accept_partner_invitation RPC
...
BEGIN
  ...
  -- Synchronize profiles.role for instructors, partner contacts, and owners
  ...
  ELSIF assigned_role = 'owner' THEN
    UPDATE profiles SET role = 'partner', updated_at = now() WHERE id = current_user_id AND role = 'user';
  ELSIF assigned_role IN ('admin', 'member') THEN
    UPDATE profiles SET role = 'contact', updated_at = now() WHERE id = current_user_id AND role = 'user';
  END IF;
...
-- 4. Update existing profiles for partner members to 'contact'
UPDATE profiles p
