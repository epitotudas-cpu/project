...
export function EditorSettingsPage() {
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      {/* Toast alert */}
      {toastMessage && (
        <div className="fixed top-4 sm:top-6 right-4 sm:right-6 z-50 flex items-center gap-3 bg-emerald-900/90 border border-emerald-500/50 text-emerald-100 px-4 sm:px-6 py-3 sm:py-4 rounded-xl shadow-2xl backdrop-blur-md animate-fade-in text-xs sm:text-sm">
      ...
      {/* Header Banner */}
      <div className={`bg-gradient-to-r ${currentPreset.bgGradient} border ${currentPreset.borderSubtle} rounded-2xl p-4 sm:p-6 md:p-8 shadow-xl transition-all duration-300`}>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 shrink-0">
            <button
              onClick={handleReset}
              className="flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl text-xs sm:text-sm font-medium transition-colors border border-slate-700"
            >
            ...
            <button
              onClick={handleSave}
              className={`flex items-center justify-center gap-2 px-6 py-2.5 ${currentPreset.buttonBg} ${currentPreset.buttonHover} ${currentPreset.buttonText} font-bold rounded-xl text-xs sm:text-sm shadow-lg ${currentPreset.shadow} transition-all hover:scale-[1.02]`}
            >
          ...
          {EDITOR_THEME_PRESETS.map((preset) => {
            ...
            return (
              <div
                ...
                className={`cursor-pointer rounded-2xl p-4 sm:p-6 border transition-all relative overflow-hidden bg-slate-900/80 ${
                  isSelected
              ...
              >
                ...
                {/* Live mini preview */}
                <div className="space-y-3 p-3.5 sm:p-4 rounded-xl bg-slate-950/60 border border-slate-800/80 text-xs">
      ...
      {/* Save Bottom Bar */}
      <div className="sticky bottom-4 sm:bottom-6 bg-slate-900/90 border border-slate-800 p-3.5 sm:p-4 rounded-2xl shadow-2xl backdrop-blur-md flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        ...
        <button
          onClick={handleSave}
          className={`flex items-center justify-center gap-2 px-6 py-2.5 ${currentPreset.buttonBg} ${currentPreset.buttonHover} ${currentPreset.buttonText} font-bold rounded-xl text-xs sm:text-sm transition-all shadow-lg ${currentPreset.shadow}`}
        >
          <Save className="w-4 h-4" />
          Mentés
        </button>
